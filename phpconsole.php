...
namespace
{
    ...
    |--------------------------------------------------------------------------
    |
    | Place your API key into the constant below. This is all you have to do
    | to start using Phpconsole. Test it with:
    |
    |     require 'phpconsole.php';
    |     p('Hello world!');
    |
    | You should be able to see data in your Phpconsole project online.
    |
    */

    const PHPCONSOLE_API_KEY = 'AIkVbqyiuMWIawayqZ0yR9xbAXQtrhadesejJTaSExeX2CylQf2Rnwzhy2rNceOu';
    ...
    |--------------------------------------------------------------------------
    |
    | Download http://curl.haxx.se/ca/cacert.pem and save it in the same
    | folder as "phpconsole.php" file, keeping the original name "cacert.pem".
    | This will allow for API address verification.
    |
    | There is no need to change any values below, unless you want to send
    | data to multiple projects or change some fine details about how
    | Phpconsole works.
    |
    */
        ...
        function p($payload, $options = array(), $metadata = array())
        {
            ...
            if (is_null($phpconsoleObject)) {
                ...
                $config->loadFromArray(array(
                    ...
                    'projects' => array(
                        'default' => array(
                            'apiKey' => PHPCONSOLE_API_KEY,
                            //'encryptionPassword' => 'passwordGoesHere'
                        ),
                    ),
                    'backtraceDepth' => 4
                ));
